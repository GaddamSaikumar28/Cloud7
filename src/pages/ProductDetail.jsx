...
//             // Sort by Price (Low to High), then Stock (High to Low to prefer in-stock)
//             allVariants.sort(([, a], [, b]) => {
//               if (a.price === b.price) return b.stock - a.stock;
...
//     // Check if current selections form a valid variant
...
//     } else {
//       // If combination is invalid (e.g. user switched flavor but old size doesn't exist),
//       // we treat it as null, but we don't clear selections immediately to avoid jumping UI.
//       setActiveVariant(null);
//     }
//   }, [selections, product]);

//   // --- LOGIC: Check if an option is valid given OTHER selections ---
//   const isOptionAvailable = (targetType, targetValue) => {
//     if (!product || !product.variantLookup) return false;

//     // We want to see if { ...otherSelections, [targetType]: targetValue } exists in any variant
//     const otherSelections = { ...selections };
//     delete otherSelections[targetType]; // Remove the type we are checking to leave only "others"

//     // Construct a partial key matcher
//     const requiredSegments = Object.entries(otherSelections).map(([k, v]) => `${k}:${v}`);
//     requiredSegments.push(`${targetType}:${targetValue}`);

//     // Check if ANY variant key contains ALL these segments
//     return Object.keys(product.variantLookup).some(key => {
//         // key format: "Flavor:Mint|Size:10"
...
//     // 2. Auto-Correct: If the new combination is invalid, try to find a valid option for the *other* types
//     // (Simple implementation: just set it, if invalid, user will see "Unavailable" or we can clear others)
...
//           {/* Dynamic Selectors with Dependency Logic */}
...
//                     // Check if this option is valid given current selections of OTHER types
...
//           {/* Accordions & Suggestions (Existing code) */}
