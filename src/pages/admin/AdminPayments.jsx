...
//     if(!window.confirm(`⚠️ Refund $${record.paid_amount} to ${record.email}? This cannot be undone.`)) return;
    
//     setProcessingId(record.order_id);
//     try {
//       await processRefund(record.order_id, record.payment_id);
...
//             {loading ? <div className="p-10 text-center text-slate-500 flex justify-center"><Loader2 className="animate-spin"/></div> : 
//              records.length === 0 ? <div className="p-10 text-center text-slate-500">No records found.</div> :
//              records.map(row => {
               
//                // --- RISK ANALYSIS LOGIC ---
//                const isPaid = row.payment_status === 'succeeded';
//                const isCancelled = row.order_status === 'cancelled';
//                const isShipped = row.order_status === 'shipped';
//                const isUnpaid = !isPaid && row.payment_status !== 'refunded';

//                // Risks
//                const refundRisk = isCancelled && isPaid; // Cancelled but we have their money
//                const revenueRisk = isShipped && isUnpaid; // Shipped but they haven't paid
...
//                        <p className="text-white font-mono text-xs">{row.order_id.substring(0,8)}...</p>
//                        <p className="text-slate-500 text-[10px] mt-1">{new Date(row.order_date).toLocaleDateString()}</p>
//                     </div>

//                     {/* 2. CUSTOMER (Now shows Email correctly!) */}
//                     <div className="col-span-3 p-4">
...
//                        <span className={`font-bold text-sm ${isPaid ? 'text-emerald-400' : 'text-slate-400'}`}>
...
//                              <span className={`font-bold uppercase ${row.order_status === 'cancelled' ? 'text-red-400' : 'text-slate-300'}`}>
//                                 {row.order_status}
//                              </span>
//                           </div>
//                           {/* Payment State */}
...
//                              title="Issue Refund"
//                              className={`p-2 rounded-lg transition-colors flex items-center gap-2 ${refundRisk ? 'bg-red-500 text-white animate-pulse shadow-lg shadow-red-500/20' : 'bg-white/5 text-slate-400 hover:text-white'}`}
//                           >
//                              <RotateCcw size={16} />
//                           </button>
//                        )}
