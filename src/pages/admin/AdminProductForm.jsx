...
// //     // 3. Filter Duplicates (Fingerprint Matching)
// //     const newRows = [];
// //     let duplicateCount = 0;

// //     combinations.forEach(comboIds => {
// //       // Create fingerprint: Sort numeric IDs to ensure order independence
// //       // Coerce to Number to handle string/number mismatches from different sources
...
// //       } else {
// //         // Unique - Add it
// //         // Resolve names safely using Number casting
...
// //       if (duplicateCount > 0) alert(`Added ${newRows.length} variants. Skipped ${duplicateCount} duplicates.`);
// //     }
...
//           if (product.product_variants) {
//              // Logic: Map DB variants to UI state
//              // Note: getProductForEdit already filters out is_active: false items
//              const variants = product.product_variants.map(v => ({
//                id: v.id, // IMPORTANT: Keeping ID allows us to update/soft-delete correctly
//                sku: v.sku,
//                price: v.price,
//                stock_quantity: v.stock_quantity,
//                optionIds: v.variant_selection_map.map(m => m.option.id),
...
//     combinations.forEach(comboIds => {
//       // Fingerprint logic to prevent duplicates
//       const newFingerprint = comboIds.map(Number).sort((a, b) => a - b).join('|');
...
//       // Save product & Handle soft deletes in API
//       await saveProduct(cleanData, generatedVariants);
...
//                      <td className="p-3"><input type="number" value={v.stock_quantity} onChange={(e) => handleVariantChange(i, 'stock_quantity', e.target.value)} className="bg-dark-950 border border-white/10 rounded p-2 text-white w-20" /></td>
//                      <td className="p-3">
//                        {/* This delete button now removes it from the LIST, and the backend handles the Soft Delete */}
//                        <button type="button" onClick={() => setGeneratedVariants(prev => prev.filter((_, idx) => idx !== i))} className="p-2 text-red-400 hover:bg-red-500/10 rounded"><Trash size={16}/></button>
//                      </td>
//                    </tr>
//                  ))}
